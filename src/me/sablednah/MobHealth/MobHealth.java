...
public class MobHealth extends JavaPlugin {
	...
	public void onEnable() {
		...
		/*
		 * if (hasHeroes && hasMobs) { logger.info("[" + myName +
		 * "] Using 'Mobs' and 'Heroes' together is NOT recommended by either plugin."); }
		 */
		if (hasMA) {
			logger.info("[" + myName + "] Monster Apocalypse Support Enabled");
		}
		if (hasZM) {
			logger.info("[" + myName + "] ZombieMod Support Enabled");
		}
		if (hasBloodMoon) {
	 ...
	 *            String of current version to compare (returned in cases such as update server is unavailable).
